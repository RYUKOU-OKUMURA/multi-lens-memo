...
export default function LensColumn({ lens, output }: LensColumnProps) {
  return (
    <section className="flex flex-col flex-1 bg-white border-r border-gray-200 min-w-0 dark:bg-gray-950 dark:border-gray-800">
      {/* カラムヘッダー */}
      <div className="flex-none flex items-center gap-2 px-3 py-2 border-b border-gray-200 dark:border-gray-800">
        ...
        {output.status === 'done' && (
          <span className="flex-none text-xs text-gray-500 dark:text-gray-700">
      ...
      <div className="flex-1 overflow-y-auto p-3">
        {output.status === 'idle' && (
          <p className="text-xs text-gray-500 italic dark:text-gray-700">
        ...
        {(output.status === 'streaming' || output.status === 'done') && (
          <div className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed dark:text-gray-200">
            {output.content}
            {output.status === 'streaming' && (
